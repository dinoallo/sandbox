...
pub trait NetOperator: Send + Sync {
    async fn delegate_ip_to_container(&self, ip: &str, container_pid: u32) -> Result<(), NetOpsError>;
}
...
impl RealNetOps {
    pub fn new() -> Self { RealNetOps {} }

    async fn create_macvlan(&self, handle: &rtnetlink::Handle, master: &str, name: &str) -> Result<u32, NetOpsError> {
        // find master
        ...
        let master_msg = master_opt.ok_or_else(|| NetOpsError::Permission(format!("master device {} not found", master)))?;
        let master_index = master_msg.header.index;

        // create macvlan; mode=0 (private) or other — passthru mode varies by kernel
        let _ = handle.link().add().macvlan(name.to_string(), master_index, 0u32).execute().await.map_err(NetOpsError::Netlink);
        ...
        Err(NetOpsError::Permission(format!("failed to create macvlan {}", name)))
    }

    async fn set_link_up_by_name(&self, handle: &rtnetlink::Handle, name: &str) -> Result<(), NetOpsError> {
        let mut links = handle.link().get().match_name(name.to_string()).execute();
        if let Some(msg) = links.try_next().await.map_err(NetOpsError::Netlink)? {
            handle.link().set(msg.header.index).up().execute().await.map_err(NetOpsError::Netlink)?;
        }
        Ok(())
    }

    async fn move_link_to_ns_by_fd(&self, handle: &rtnetlink::Handle, idx: u32, fd: i32) -> Result<(), NetOpsError> {
        handle.link().set(idx).setns_by_fd(fd).execute().await.map_err(NetOpsError::Netlink)?;
        Ok(())
    }

    async fn add_addr_by_name(&self, handle: &rtnetlink::Handle, ifname: &str, addr: std::net::IpAddr, prefix: u8) -> Result<(), NetOpsError> {
        let mut links = handle.link().get().match_name(ifname.to_string()).execute();
        if let Some(msg) = links.try_next().await.map_err(NetOpsError::Netlink)? {
            handle.address().add(msg.header.index, addr, prefix).execute().await.map_err(NetOpsError::Netlink)?;
        }
        Ok(())
    }
}
...
impl NetOperator for RealNetOps {
    async fn delegate_ip_to_container(&self, ip: &str, container_pid: u32) -> Result<(), NetOpsError> {
        ...
        if !ns_path.exists() { return Err(NetOpsError::Permission(format!("netns path not found: {}", ns_path.display()))); }
        ...
        if let Ok(s) = handle.link().get().match_name("eth0".to_string()).execute().try_next().await {
            if let Some(msg) = s { let _ = handle.link().set(msg.header.index).promiscuous(true).execute().await; }
        }
        ...
        setns(&ns_file, CloneFlags::CLONE_NEWNET).map_err(|e| NetOpsError::Permission(format!("setns failed: {}", e)))?;
        ...
        if let Err(e) = self.set_link_up_by_name(&cn_handle, "eth1").await {
            ...
            if let Ok(Some(msg)) = cn_handle.link().get().match_name("eth1".to_string()).execute().try_next().await {
                let _ = cn_handle.link().del(msg.header.index).execute().await;
            }
            // restore ns and return
            setns(&original, CloneFlags::CLONE_NEWNET).map_err(|e| NetOpsError::Permission(format!("restore setns failed: {}", e)))?;
            return Err(e);
        }

        if let Err(e) = self.add_addr_by_name(&cn_handle, "eth1", addr, prefix).await {
...
impl MockNetOps { pub fn new() -> Self { MockNetOps{} } }
...
pub async fn delegate_ip_to_container(ip: &str, container_pid: u32) -> Result<(), NetOpsError> {
    let use_real = std::env::var("USE_REAL_NETOPS").map(|v| v == "1" || v.to_lowercase() == "true").unwrap_or(false);
    if use_real {
...
fn parse_ip_with_prefix(s: &str) -> Result<(std::net::IpAddr, u8), NetOpsError> {
    if let Some(pos) = s.find('/') {
        ...
        let pref = pref_str.trim_start_matches('/').parse::<u8>().map_err(|e| NetOpsError::Permission(format!("invalid prefix: {}", e)))?;
        let addr = addr_str.parse::<std::net::IpAddr>().map_err(|e| NetOpsError::Permission(format!("invalid ip: {}", e)))?;
        Ok((addr, pref))
    } else {
        Err(NetOpsError::Permission("ip must be in form x.x.x.x/prefix".into()))
    }
}
...
mod tests {
    ...
    async fn delegate_self_ns_ok() {
        ...
        assert!(res.is_ok(), "expected mock delegate to succeed for current process ns");
    }
}

...
use crate::launcher::{CreateRequest, CreateResponse, DeleteRequest, DeleteResponse, PingRequest, PingResponse};
...
impl crate::launcher::launcher_server::Launcher for LauncherService {
    async fn create(&self, req: Request<CreateRequest>) -> Result<Response<CreateResponse>, Status> {
        ...
        let image = if req.image.is_empty() { "ubuntu:22.04".to_string() } else { req.image.clone() };

        // Basic serialized handling so the mock doesn't race
        let _guard = self.state.lock().await;

        self.client.create_container(&name, &image).await.map_err(|e| Status::internal(format!("create failed: {}", e)))?;

        // Wait for container to be running and get main pid
        let pid = self.client.wait_for_pid(&name, Duration::from_secs(10)).await.map_err(|e| Status::internal(format!("wait for pid failed: {}", e)))?;
        ...
        Ok(Response::new(CreateResponse { success: true, message: format!("container {} created", name) }))
    }
    ...
    async fn delete(&self, req: Request<DeleteRequest>) -> Result<Response<DeleteResponse>, Status> {
        ...
        self.client.delete_container(&name).await.map_err(|e| Status::internal(format!("delete failed: {}", e)))?;

        // Wait for shutdown
        self.client.wait_for_shutdown(&name, Duration::from_secs(10)).await.map_err(|e| Status::internal(format!("wait shutdown failed: {}", e)))?;

        Ok(Response::new(DeleteResponse { success: true, message: format!("container {} deleted", name) }))
    }
    ...
    async fn ping(&self, req: Request<PingRequest>) -> Result<Response<PingResponse>, Status> {
        ...
        Ok(Response::new(PingResponse { ok: true, message: "pong".to_string() }))
    }
}

impl Clone for LauncherService {
    fn clone(&self) -> Self {
        Self { client: self.client.clone_box(), state: self.state.clone() }
    }
}
...
mod tests {
    ...
    async fn create_without_ip_succeeds() {
        ...
        let req = Request::new(CreateRequest { name: "mytest".to_string(), image: "".to_string(), ip: "".to_string() });
        // trait needs to be in scope so we can call the async fn on the impl
        let resp = LauncherTrait::create(&svc, req).await.expect("create failed");
    ...
    async fn delete_succeeds() {
        ...
        let req = Request::new(DeleteRequest { name: "mytest".to_string() });
        let resp = LauncherTrait::delete(&svc, req).await.expect("delete failed");
        let inner = resp.into_inner();
